...
__global__ void cuda_trmm(int m0, int n0, float *A_device, float *B_device, float *C_device) {
    ...
    */
    ...
    int j0 = blockIdx.x * BLOCK_SIZE + (threadIdx.x / BLOCK_SIZE);
    int i0 = blockIdx.y * BLOCK_SIZE + (threadIdx.x % BLOCK_SIZE);

    // same as simple matrix multiply kernel... only with the added j0 > i0 condition
...

{
    ...
    // ceiling division so that we still create grid blocks if not nice even division
    dim3 grid_dim((m0 + BLOCK_SIZE - 1) / BLOCK_SIZE, (n0 + BLOCK_SIZE - 1) / BLOCK_SIZE);
