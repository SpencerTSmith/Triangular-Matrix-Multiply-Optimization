...
#pragma omp parallel for collapse(2)
        for (int i_block = 0; i_block < m0; i_block += block_size) {  // Parallelize block loop
            for (int i0 = i_block; i0 < i_block + block_size; ++i0) { // Loop through block
                for (int j0 = i0 + 1; j0 < n0; ++j0) {                // Upper triangular condition
                    float res = 0.0f;
                    int p0;

                    for (p0 = 0; p0 < m0 - 1; p0 += 2) { // Compute dot product in pairs
                    ...
                    C_distributed[i0 * cs_C + j0 * rs_C] = res; // Store result
                }
            }
        }
    }
