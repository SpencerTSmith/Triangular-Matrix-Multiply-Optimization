        ...
        for (int i_block = 0; i_block < m0; i_block += block_size) {
            for (int j_block = i_block + 1; j_block < n0; j_block += block_size) {
                for (int p_block = 0; p_block < m0; p_block += block_size) {
                    ...
                    for (int ii_block = i_block; ii_block < i_block + block_size && ii_block < m0;
                         ii_block += sub_block_size) {
                        for (int jj_block = j_block;
                             jj_block < j_block + block_size && jj_block < n0;
                             jj_block += sub_block_size) {
                            for (int pp_block = p_block;
                                 pp_block < p_block + block_size && pp_block < m0;
                                 pp_block += sub_block_size) {

                                // Inner loops for elements within each sub-block
                                for (int i0 = ii_block; i0 < ii_block + sub_block_size && i0 < m0;
                                     ++i0) {
                                    for (int j0 = jj_block;
                                         j0 < jj_block + sub_block_size && j0 < n0; ++j0) {
                                        if (j0 > i0) { // Maintain triangular matrix constraint
                                            float res = 0.0f;

                                            // Unrolling the p0 loop by a factor of 4
                                            int p0 = pp_block;
                                            for (; p0 <= pp_block + sub_block_size - 4 && p0 < m0;
                                                 p0 += 4) {
                                            ...
                                            for (; p0 < pp_block + sub_block_size && p0 < m0;
                                                 ++p0) {
