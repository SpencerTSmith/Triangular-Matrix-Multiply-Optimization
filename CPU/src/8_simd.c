    ...
    if (rid == root_rid) {
        for (int i0 = 0; i0 < m0; ++i0) {
            for (int j0 = i0 + 1; j0 < n0; ++j0) {
                ...
                for (int p0 = 0; p0 < m0; p0 += AVX2_FLOAT_N) {
                    __m256 A_row_reg = _mm256_loadu_ps(&A_distributed[i0 * cs_A + p0]);
                    __m256 B_col_reg = _mm256_loadu_ps(&B_distributed[j0 * rs_B + p0]);

                    res = _mm256_fmadd_ps(A_row_reg, B_col_reg, res);
                }
                ...
                __m128 low_lane = _mm256_castps256_ps128(res);
                __m128 high_lane = _mm256_extractf128_ps(res, 0b1);
